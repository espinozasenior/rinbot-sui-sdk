...
const swapPatterns: Record<string, `${string}::${string}::${string}`> = {
  ".*::router::swap_exact_output$": `${DCA_CONTRACT}::${"flowx"}::${"swap_exact_output"}`,
  ".*::router::swap_exact_output_doublehop$": `${DCA_CONTRACT}::${"flowx"}::${"swap_exact_output_doublehop"}`,
  ".*::router::swap_exact_output_triplehop$": `${DCA_CONTRACT}::${"flowx"}::${"swap_exact_output_triplehop"}`,
  ".*::router::swap_exact_input$": `${DCA_CONTRACT}::${"flowx"}::${"swap_exact_input"}`,
  ".*::router::swap_exact_input_doublehop$": `${DCA_CONTRACT}::${"flowx"}::${"swap_exact_input_doublehop"}`,
  ".*::router::swap_exact_input_triplehop$": `${DCA_CONTRACT}::${"flowx"}::${"swap_exact_input_triplehop"}`,
};
...
// type SwapParamsExactOut = {
//   clock: Argument;
//   pools: Argument;
//   inputFunds: Argument;
//   maxInputAmount: Argument;
//   exactOutput: Argument;
//   recipient: Argument;
//   sqrtPrice: Argument;
//   dca: Argument;
//   gasGost: Argument;
  ...
  txBlock.blockData.transactions.forEach((transaction) => {
    if (transaction.kind === "MoveCall" && transaction.target) {
      ...
      if (swapMatch) {
        ...
        const newTarget: `${string}::${string}::${string}` = `${DCA_CONTRACT}::${"flowx"}::${parts[2]}`;
        isExactIn = parts[2].includes("exact_input");

        const swapParams: SwapParams = isExactIn
          ? {
              clock: fromArgument(transaction.arguments[0] as Argument, 0),
              pools: fromArgument(transaction.arguments[1] as Argument, 1),
              inputFunds: fromArgument(transaction.arguments[2] as Argument, 2),
              minOutput: fromArgument(transaction.arguments[3] as Argument, 3),
              recipient: fromArgument(transaction.arguments[4] as Argument, 4),
              sqrtPrice: fromArgument(transaction.arguments[5] as Argument, 5),
              dca: { kind: "Input", value: dcaId, index: 6, type: "object" },
              gasGost: { kind: "Input", value: gasCost, index: 7, type: "pure" },
            }
          : {
              clock: fromArgument(transaction.arguments[0] as Argument, 0),
              pools: fromArgument(transaction.arguments[1] as Argument, 1),
              inputFunds: fromArgument(transaction.arguments[2] as Argument, 2),
              maxInputAmount: fromArgument(transaction.arguments[3] as Argument, 3),
              exactOutput: fromArgument(transaction.arguments[4] as Argument, 4),
              recipient: fromArgument(transaction.arguments[5] as Argument, 5),
              sqrtPrice: fromArgument(transaction.arguments[6] as Argument, 6),
              dca: { kind: "Input", value: dcaId, index: 7, type: "object" },
              gasGost: { kind: "Input", value: gasCost, index: 8, type: "pure" },
            };
        ...
        const inputs = isExactIn
          ? [
              ...
              swapParams.inputFunds as Input,
